...
function publicarAsignatura() {
	...
	return deployContract('SubjectToken', accountUniversity, ABI_Subject,
		DATA_Subject, [subjectname, symbol, limitmint,
		expirationtime, price, descriptionURI]).then(z => {
			let accountSCSubject = z._address;
			let SCSubjectToken = new web3.eth.Contract(ABI_Subject, accountSCSubject);
			return SCSubjectToken.methods.transferOwnership(accountSCUniversity).send({
				gas: 3000000,
				from: accountUniversity
			}).then(n => {
				let SCUniversity = new web3.eth.Contract(ABI_University, accountSCUniversity);
				return SCUniversity.methods.addSubject(subjectname, accountSCSubject).send({
					gas: 6000000,
					from: accountUniversity
				});
			});
...
async function NewAccount(password) {
				let result = await web3.eth.personal.newAccount(password).then(console.log);
				return result;
			}
